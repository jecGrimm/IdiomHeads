...
if __name__ == "__main__":
    ...
    if tensor_file.endswith("_comp.pt"):
        ...
        #plot_logit_attribution_split(loaded_tensor, x_labels = x_labels[model_name], filename=img_file)
        mean_tensor = get_mean_sentence_tensor(loaded_tensor)
        print("Size of mean tensor:", mean_tensor.size())
        print("Mean tensor 0:", mean_tensor[0])
        print("Mean tensor 1:", mean_tensor[1])
        # df = pd.DataFrame(mean_tensor.numpy().T, columns=['Idiom', 'Literal'])
        # df.plot(ax=plt.gca())
        # #df.plot.line()
        # plt.show()
    else:
