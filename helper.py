...
def merge_tensors(file1: str, file2: str, outfile: str):
    """
    This function merges concats two tensors.

    @params
        file1: file of the first tensor
        file2: file of the second tensor
        outfile: file of the resulting tensor
    """
...
def tranform_comp2score(file: str, outfile: str):
    """
    This function transforms the Idiom or Literal Score components to the final score.
    ...
    """
    device = "cuda" if t.cuda.is_available() else "cpu"
    tensor = t.load(file, map_location=t.device(device))
    print("Size tensor1: ", tensor.size())

    score_tensor = 2 * (t.sigmoid(t.sum(tensor, dim = -1))-0.5)
    print("Size score_tensor: ", score_tensor.size())

    t.save(score_tensor, outfile)

def print_tensor_size(file: str):
    """
    This function prints the size of a tensor.
...
def get_logit_component(num: int, model_name: str):
    """
    This function returns the component name for the position of a component.
    ...
    start = -1
    for comp_group in x_labels[model_name.lower()]:
...
if __name__ == "__main__":
    comp_file = "future_work/scores/literal_scores/Pythia-1.4B/literal_only_static_0_2761_comp.pt"
    tranform_comp2score(comp_file, "future_work/scores/literal_scores/Pythia-1.4B/literal_static_0_2761.pt")